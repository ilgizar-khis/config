...
"
" 256-color terminal
fileviewer *.[ch],*.[ch]pp,*.py,*.json highlight -O xterm256 -s olive --syntax c %c
fileviewer Makefile,Makefile.* highlight -O xterm256 -s olive --syntax make %c
"
" 16-color terminal
...
" Preview common image formats using kitty's icat
fileviewer {*.bmp,*.jpg,*.jpeg,*.png,*.gif,*.xpm},<image/*>
    \ kitty +icat --silent --transfer-mode=memory --place=%pwx%ph@%pxx%py %c >/dev/tty </dev/tty %N
    \ %pc
    \ kitty icat --clear --silent >/dev/tty </dev/tty %N
